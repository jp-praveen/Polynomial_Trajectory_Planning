...
% and heading is also included in this script
...
tnodes =[optimvar('t0',1, 1, 'LowerBound', ti, 'UpperBound', ti);
         optimvar('t1',1, 1, 'LowerBound', ti+1, 'UpperBound', ti+1.5);
         optimvar('t2',1, 1, 'LowerBound', ti+2.5, 'UpperBound', ti+3);
         optimvar('t3',1, 1, 'LowerBound', ti+4, 'UpperBound', ti+5)].';
...
C = [optimvar('c0',1, 4*m, 'LowerBound', -10000000, 'UpperBound', 10000000);
     optimvar('c1',1, 4*m, 'LowerBound', -10000000, 'UpperBound', 10000000);
     optimvar('c2',1, 4*m, 'LowerBound', -10000000, 'UpperBound', 10000000);
     optimvar('c3',1, 4*m, 'LowerBound', -10000000, 'UpperBound', 10000000);
     optimvar('c4',1, 4*m, 'LowerBound', -10000000, 'UpperBound', 10000000);
     optimvar('c5',1, 4*m, 'LowerBound', -10000000, 'UpperBound', 10000000);
     optimvar('c6',1, 4*m, 'LowerBound', -10000000, 'UpperBound', 10000000);
     optimvar('c7',1, 4*m, 'LowerBound', -10000000, 'UpperBound', 10000000)];
...
for i = 1:m
    ...
    PosNodeOddang(i) = C(1,p)*tarrPos(1,i)*tnodes(i)^0 + tarrPos(2,i)*C(2,p)*tnodes(i)^1 + tarrPos(3,i)*C(3,p)*tnodes(i)^2 + tarrPos(4,i)*C(4,p)*tnodes(i)^3 + tarrPos(5,i)*C(5,p)*tnodes(i)^4 +tarrPos(6,i)*C(6,p)*tnodes(i)^5 + tarrPos(7,i)*C(7,p)*tnodes(i)^6 +tarrPos(8,i)*C(8,p)*tnodes(i)^7 == angnodes(i);
end
...
for i = 1:m
    ...
    PosNodeEvenang(i) = C(1,p)*tarrPos(1,i+1)*tnodes(i+1)^0 + tarrPos(2,i+1)*C(2,p)*tnodes(i+1)^1 + tarrPos(3,i+1)*C(3,p)*tnodes(i+1)^2 + tarrPos(4,i+1)*C(4,p)*tnodes(i+1)^3 + tarrPos(5,i+1)*C(5,p)*tnodes(i+1)^4 +tarrPos(6,i+1)*C(6,p)*tnodes(i+1)^5 + tarrPos(7,i+1)*C(7,p)*tnodes(i+1)^6 +tarrPos(8,i+1)*C(8,p)*tnodes(i+1)^7 == angnodes(i+1);
end
...
MinimumSnap.Constraints.PosNodeOddang = PosNodeOddang; 
MinimumSnap.Constraints.PosNodeEvenang = PosNodeEvenang; 
...
VelNode1ang = tarrVel(1,1)*C(1,1+3*m)*tnodes(1)^0 + tarrVel(2,1)*C(2,1+3*m)*tnodes(1)^0 + tarrVel(3,1)*C(3,1+3*m)*tnodes(1)^1 + tarrVel(4,1)*C(4,1+3*m)*tnodes(1)^2 + tarrVel(5,1)*C(5,1+3*m)*tnodes(1)^3 +tarrVel(6,1)*C(6,1+3*m)*tnodes(1)^4 + tarrVel(7,1)*C(7,1+3*m)*tnodes(1)^5 +tarrVel(8,1)*C(8,1+3*m)*tnodes(1)^6 == 0;
VelNodefang = tarrVel(1,end)*C(1,4*m)*tnodes(end)^0 + tarrVel(2,end)*C(2,4*m)*tnodes(end)^0 + tarrVel(3,end)*C(3,4*m)*tnodes(end)^1 + tarrVel(4,end)*C(4,4*m)*tnodes(end)^2 + tarrVel(5,end)*C(5,4*m)*tnodes(end)^3 +tarrVel(6,end)*C(6,4*m)*tnodes(end)^4 + tarrVel(7,end)*C(7,4*m)*tnodes(end)^5 +tarrVel(8,end)*C(8,4*m)*tnodes(end)^6 == 0;
...
MinimumSnap.Constraints.VelNode1ang = VelNode1ang;
MinimumSnap.Constraints.VelNodefang = VelNodefang;
...
AccNode1ang = tarrAcc(1,1)*C(1,1+3*m)*tnodes(1)^0 + tarrAcc(2,1)*C(2,1+3*m)*tnodes(1)^0 + tarrAcc(3,1)*C(3,1+3*m)*tnodes(1)^1 + tarrAcc(4,1)*C(4,1+3*m)*tnodes(1)^2 + tarrAcc(5,1)*C(5,1+3*m)*tnodes(1)^3 +tarrAcc(6,1)*C(6,1+3*m)*tnodes(1)^4 + tarrAcc(7,1)*C(7,1+3*m)*tnodes(1)^5 +tarrAcc(8,1)*C(8,1+3*m)*tnodes(1)^6 == 0;
AccNodefang = tarrAcc(1,end)*C(1,4*m)*tnodes(end)^0 + tarrAcc(2,end)*C(2,4*m)*tnodes(end)^0 + tarrAcc(3,end)*C(3,4*m)*tnodes(end)^1 + tarrAcc(4,end)*C(4,4*m)*tnodes(end)^2 + tarrAcc(5,end)*C(5,4*m)*tnodes(end)^3 +tarrAcc(6,end)*C(6,4*m)*tnodes(end)^4 + tarrAcc(7,end)*C(7,4*m)*tnodes(end)^5 +tarrAcc(8,end)*C(8,4*m)*tnodes(end)^6 == 0;
...
MinimumSnap.Constraints.AccNode1ang = AccNode1ang;
MinimumSnap.Constraints.AccNodefang = AccNodefang;
...
JerkNode1ang = tarrJerk(1,1)*C(1,1+3*m)*tnodes(1)^0 + tarrJerk(2,1)*C(2,1+3*m)*tnodes(1)^0 + tarrJerk(3,1)*C(3,1+3*m)*tnodes(1)^1 + tarrJerk(4,1)*C(4,1+3*m)*tnodes(1)^2 + tarrJerk(5,1)*C(5,1+3*m)*tnodes(1)^3 +tarrJerk(6,1)*C(6,1+3*m)*tnodes(1)^4 + tarrJerk(7,1)*C(7,1+3*m)*tnodes(1)^5 +tarrJerk(8,1)*C(8,1+3*m)*tnodes(1)^6 == 0;
JerkNodefang = tarrJerk(1,end)*C(1,4*m)*tnodes(end)^0 + tarrJerk(2,end)*C(2,4*m)*tnodes(end)^0 + tarrJerk(3,end)*C(3,4*m)*tnodes(end)^1 + tarrJerk(4,end)*C(4,4*m)*tnodes(end)^2 + tarrJerk(5,end)*C(5,4*m)*tnodes(end)^3 +tarrJerk(6,end)*C(6,4*m)*tnodes(end)^4 + tarrJerk(7,end)*C(7,4*m)*tnodes(end)^5 +tarrJerk(8,end)*C(8,4*m)*tnodes(end)^6 == 0;
...
MinimumSnap.Constraints.JerkNode1ang = JerkNode1ang;
MinimumSnap.Constraints.JerkNodefang = JerkNodefang;
...
%     SmoothVely(i) = C(:,m+i)'*tarrVel(:,i+1) == C(:,m+i+1)'*tarrVel(:,i+1);
    ...
    SmoothVelang(i) = tarrVel(1,i+1)*C(1,3*m+i)*tnodes(i+1)^0 + tarrVel(2,i+1)*C(2,3*m+i)*tnodes(i+1)^0 + tarrVel(3,i+1)*C(3,3*m+i)*tnodes(i+1)^1 + tarrVel(4,i+1)*C(4,3*m+i)*tnodes(i+1)^2 + tarrVel(5,i+1)*C(5,3*m+i)*tnodes(i+1)^3 +tarrVel(6,i+1)*C(6,3*m+i)*tnodes(i+1)^4 + tarrVel(7,i+1)*C(7,3*m+i)*tnodes(i+1)^5 +tarrVel(8,i+1)*C(8,3*m+i)*tnodes(i+1)^6 == tarrVel(1,i+1)*C(1,3*m+i+1)*tnodes(i+1)^0 + tarrVel(2,i+1)*C(2,3*m+i+1)*tnodes(i+1)^0 + tarrVel(3,i+1)*C(3,3*m+i+1)*tnodes(i+1)^1 + tarrVel(4,i+1)*C(4,3*m+i+1)*tnodes(i+1)^2 + tarrVel(5,i+1)*C(5,3*m+i+1)*tnodes(i+1)^3 +tarrVel(6,i+1)*C(6,3*m+i+1)*tnodes(i+1)^4 + tarrVel(7,i+1)*C(7,3*m+i+1)*tnodes(i+1)^5 +tarrVel(8,i+1)*C(8,3*m+i+1)*tnodes(i+1)^6;
    SmoothAccang(i) = tarrAcc(1,i+1)*C(1,3*m+i)*tnodes(i+1)^0 + tarrAcc(2,i+1)*C(2,3*m+i)*tnodes(i+1)^0 + tarrAcc(3,i+1)*C(3,3*m+i)*tnodes(i+1)^1 + tarrAcc(4,i+1)*C(4,3*m+i)*tnodes(i+1)^2 + tarrAcc(5,i+1)*C(5,3*m+i)*tnodes(i+1)^3 +tarrAcc(6,i+1)*C(6,3*m+i)*tnodes(i+1)^4 + tarrAcc(7,i+1)*C(7,3*m+i)*tnodes(i+1)^5 +tarrAcc(8,i+1)*C(8,3*m+i)*tnodes(i+1)^6 == tarrAcc(1,i+1)*C(1,3*m+i+1)*tnodes(i+1)^0 + tarrAcc(2,i+1)*C(2,3*m+i+1)*tnodes(i+1)^0 + tarrAcc(3,i+1)*C(3,3*m+i+1)*tnodes(i+1)^1 + tarrAcc(4,i+1)*C(4,3*m+i+1)*tnodes(i+1)^2 + tarrAcc(5,i+1)*C(5,3*m+i+1)*tnodes(i+1)^3 +tarrAcc(6,i+1)*C(6,3*m+i+1)*tnodes(i+1)^4 + tarrAcc(7,i+1)*C(7,3*m+i+1)*tnodes(i+1)^5 +tarrAcc(8,i+1)*C(8,3*m+i+1)*tnodes(i+1)^6;
    SmoothJerkang(i) = tarrJerk(1,i+1)*C(1,3*m+i)*tnodes(i+1)^0 + tarrJerk(2,i+1)*C(2,3*m+i)*tnodes(i+1)^0 + tarrJerk(3,i+1)*C(3,3*m+i)*tnodes(i+1)^1 + tarrJerk(4,i+1)*C(4,3*m+i)*tnodes(i+1)^2 + tarrJerk(5,i+1)*C(5,3*m+i)*tnodes(i+1)^3 +tarrJerk(6,i+1)*C(6,3*m+i)*tnodes(i+1)^4 + tarrJerk(7,i+1)*C(7,3*m+i)*tnodes(i+1)^5 +tarrJerk(8,i+1)*C(8,3*m+i)*tnodes(i+1)^6 == tarrJerk(1,i+1)*C(1,3*m+i+1)*tnodes(i+1)^0 + tarrJerk(2,i+1)*C(2,3*m+i+1)*tnodes(i+1)^0 + tarrJerk(3,i+1)*C(3,3*m+i+1)*tnodes(i+1)^1 + tarrJerk(4,i+1)*C(4,3*m+i+1)*tnodes(i+1)^2 + tarrJerk(5,i+1)*C(5,3*m+i+1)*tnodes(i+1)^3 +tarrJerk(6,i+1)*C(6,3*m+i+1)*tnodes(i+1)^4 + tarrJerk(7,i+1)*C(7,3*m+i+1)*tnodes(i+1)^5 +tarrJerk(8,i+1)*C(8,3*m+i+1)*tnodes(i+1)^6;
    SmoothSnapang(i) = tarrSnap(1,i+1)*C(1,3*m+i)*tnodes(i+1)^0 + tarrSnap(2,i+1)*C(2,3*m+i)*tnodes(i+1)^0 + tarrSnap(3,i+1)*C(3,3*m+i)*tnodes(i+1)^1 + tarrSnap(4,i+1)*C(4,3*m+i)*tnodes(i+1)^2 + tarrSnap(5,i+1)*C(5,3*m+i)*tnodes(i+1)^3 +tarrSnap(6,i+1)*C(6,3*m+i)*tnodes(i+1)^4 + tarrSnap(7,i+1)*C(7,3*m+i)*tnodes(i+1)^5 +tarrSnap(8,i+1)*C(8,3*m+i)*tnodes(i+1)^6 == tarrSnap(1,i+1)*C(1,3*m+i+1)*tnodes(i+1)^0 + tarrSnap(2,i+1)*C(2,3*m+i+1)*tnodes(i+1)^0 + tarrSnap(3,i+1)*C(3,3*m+i+1)*tnodes(i+1)^1 + tarrSnap(4,i+1)*C(4,3*m+i+1)*tnodes(i+1)^2 + tarrSnap(5,i+1)*C(5,3*m+i+1)*tnodes(i+1)^3 +tarrSnap(6,i+1)*C(6,3*m+i+1)*tnodes(i+1)^4 + tarrSnap(7,i+1)*C(7,3*m+i+1)*tnodes(i+1)^5 +tarrSnap(8,i+1)*C(8,3*m+i+1)*tnodes(i+1)^6;
    SmoothFiveang(i) = tarrFive(1,i+1)*C(1,3*m+i)*tnodes(i+1)^0 + tarrFive(2,i+1)*C(2,3*m+i)*tnodes(i+1)^0 + tarrFive(3,i+1)*C(3,3*m+i)*tnodes(i+1)^1 + tarrFive(4,i+1)*C(4,3*m+i)*tnodes(i+1)^2 + tarrFive(5,i+1)*C(5,3*m+i)*tnodes(i+1)^3 +tarrFive(6,i+1)*C(6,3*m+i)*tnodes(i+1)^4 + tarrFive(7,i+1)*C(7,3*m+i)*tnodes(i+1)^5 +tarrFive(8,i+1)*C(8,3*m+i)*tnodes(i+1)^6 == tarrFive(1,i+1)*C(1,3*m+i+1)*tnodes(i+1)^0 + tarrFive(2,i+1)*C(2,3*m+i+1)*tnodes(i+1)^0 + tarrFive(3,i+1)*C(3,3*m+i+1)*tnodes(i+1)^1 + tarrFive(4,i+1)*C(4,3*m+i+1)*tnodes(i+1)^2 + tarrFive(5,i+1)*C(5,3*m+i+1)*tnodes(i+1)^3 +tarrFive(6,i+1)*C(6,3*m+i+1)*tnodes(i+1)^4 + tarrFive(7,i+1)*C(7,3*m+i+1)*tnodes(i+1)^5 +tarrFive(8,i+1)*C(8,3*m+i+1)*tnodes(i+1)^6;
    SmoothSixang(i) = tarrSix(1,i+1)*C(1,3*m+i)*tnodes(i+1)^0 + tarrSix(2,i+1)*C(2,3*m+i)*tnodes(i+1)^0 + tarrSix(3,i+1)*C(3,3*m+i)*tnodes(i+1)^1 + tarrSix(4,i+1)*C(4,3*m+i)*tnodes(i+1)^2 + tarrSix(5,i+1)*C(5,3*m+i)*tnodes(i+1)^3 +tarrSix(6,i+1)*C(6,3*m+i)*tnodes(i+1)^4 + tarrSix(7,i+1)*C(7,3*m+i)*tnodes(i+1)^5 +tarrSix(8,i+1)*C(8,3*m+i)*tnodes(i+1)^6 == tarrSix(1,i+1)*C(1,3*m+i+1)*tnodes(i+1)^0 + tarrSix(2,i+1)*C(2,3*m+i+1)*tnodes(i+1)^0 + tarrSix(3,i+1)*C(3,3*m+i+1)*tnodes(i+1)^1 + tarrSix(4,i+1)*C(4,3*m+i+1)*tnodes(i+1)^2 + tarrSix(5,i+1)*C(5,3*m+i+1)*tnodes(i+1)^3 +tarrSix(6,i+1)*C(6,3*m+i+1)*tnodes(i+1)^4 + tarrSix(7,i+1)*C(7,3*m+i+1)*tnodes(i+1)^5 +tarrSix(8,i+1)*C(8,3*m+i+1)*tnodes(i+1)^6;
...
MinimumSnap.Constraints.SmoothVelang = SmoothVelang;
MinimumSnap.Constraints.SmoothAccang = SmoothAccang;
MinimumSnap.Constraints.SmoothJerkang = SmoothJerkang;
MinimumSnap.Constraints.SmoothSnapang = SmoothSnapang;
MinimumSnap.Constraints.SmoothFiveang = SmoothFiveang;
MinimumSnap.Constraints.SmoothSixang = SmoothSixang;
...
Cost = Ccostx'*H*Ccostx + Ccosty'*H*Ccosty + Ccostz'*H*Ccostz + Ccostang'*H*Ccostang + sum(tnodes*tnodes.'); 
...
Z_Guess.c0 = 10*ones(4*m,1);
Z_Guess.c1 = 10*ones(4*m,1);
Z_Guess.c2 = 10*ones(4*m,1);
Z_Guess.c3 = 10*ones(4*m,1);
Z_Guess.c4 = 10*ones(4*m,1);
Z_Guess.c5 = 10*ones(4*m,1);
Z_Guess.c6 = 10*ones(4*m,1);
Z_Guess.c7 = 10*ones(4*m,1);
...
for i = 1:nsub
    ...
    angS1(i) = C0(10) + C1(10)*timeS1(i) + C2(10)*timeS1(i)^2 + C3(10)*timeS1(i)^3 + C4(10)*timeS1(i)^4 + C5(10)*timeS1(i)^5 + C6(10)*timeS1(i)^6 + C7(10)*timeS1(i)^7; 
    angS2(i) = C0(11) + C1(11)*timeS2(i) + C2(11)*timeS2(i)^2 + C3(11)*timeS2(i)^3 + C4(11)*timeS2(i)^4 + C5(11)*timeS2(i)^5 + C6(11)*timeS2(i)^6 + C7(11)*timeS2(i)^7; 
    angS3(i) = C0(12) + C1(12)*timeS3(i) + C2(12)*timeS3(i)^2 + C3(12)*timeS3(i)^3 + C4(12)*timeS3(i)^4 + C5(12)*timeS3(i)^5 + C6(12)*timeS3(i)^6 + C7(12)*timeS3(i)^7;
